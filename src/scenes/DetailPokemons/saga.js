/*
    Screen responsible for store sagas (for DetailPokemons screen), with their parameters and types
...
import {
    GET_DETAIL_POKEMON,
    successGetDetailPokemons,
    errorGetDetailPokemons
} from './action'

import { 
    transformDetailsPokemons
} from './utils'
...
function * getDetailsPokemons({ payload }){
    try {
        ...
        const response = yield call(axios.get, `${baseUrl}/${detail.id}`)
        if (response && response.status === 200){
            yield put(successGetDetailPokemons(transformDetailsPokemons(response.data, favouritePokemons)))
        }else{
            cogoToast.error(response.data.message || 'Not Possible at the moment. Please Contact support.')
        }
    }catch(e){
        yield put(errorGetDetailPokemons(e.response.data.errorMessage))
        if (e.response) {
...
export default function * watcherDetailPokemons(){
    yield takeLatest(GET_DETAIL_POKEMON, getDetailsPokemons)
}